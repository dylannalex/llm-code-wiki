# LLM Code Wiki — Schema (constitution)
...
> **This file is the single source of truth (the "constitution").** It defines how the wiki is
> structured and what workflows to follow. It is delivered two ways: the global `wiki` skill
> reads it on every invocation (so the wiki is usable from any repo), and the repo's short
> `CLAUDE.md` redirects here when you work inside this repo. Co-evolve it with the human as
> conventions improve — but keep it the *only* place the schema lives; the skill and `CLAUDE.md`
> must point here, never copy.
...
## Invocation & path anchoring (read this first)

This wiki is used **primarily from other repos** via the global `wiki` skill, so it cannot rely on
being the current working directory. **Anchor every wiki operation to the wiki root** — the
absolute path the skill was installed with (call it `<WIKI_ROOT>`). Index reads, page paths,
`repos.md` resolution, and the hash-script staleness checks all resolve against `<WIKI_ROOT>`,
**not** the repo you happen to be invoked from. The repo you are currently in may be a *source*
for the wiki (register it in [[repos]]), but the wiki itself always lives at `<WIKI_ROOT>`.
...
repo root (machinery — keep out of the vault):
  ...
  constitution.md    THIS schema — the single source of truth (read by the wiki skill + CLAUDE.md)
  CLAUDE.md          short in-repo pointer that redirects here (auto-loads only inside this repo)
...
All paths below are relative to `<WIKI_ROOT>` (see *Invocation & path anchoring* above).
